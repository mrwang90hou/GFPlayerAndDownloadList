...
- (NSURL *)getMediaURL:(NSString *)urlStr
{
    ...
    for (NSDictionary *dic in array) {
        if ([urlStr rangeOfString:dic[@"url"]].location != NSNotFound) {
            NSString *filePath = dic[@"fileName"];
            if (filePath.length > 0) {
                _isLocateVideo = YES;//是否是已下载视频播放
                _videoBtn.hidden = YES;
                NSLog(@"当前为已下载视频播放！！！\nfilePath = %@",dic[@"fileName"]);
//                NSURL * filePathURL = [NSURL fileURLWithPath:dic[@"fileName"]];
//                NSLog(@"filePathURL = %@",filePathURL);
//                NSURL * filePathURL =[NSURL fileURLWithPath:[NSString stringWithFormat:@"/private%@",dic[@"fileName"]]];
//                return filePathURL;
